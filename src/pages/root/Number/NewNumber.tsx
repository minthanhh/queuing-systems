        ...
        <>
            <div className="mt-4 mb-[42px] px-6 w-full">
                ...
                <div className="bg-white rounded-lg shadow-sm flex-1 flex items-center flex-col w-full h-[500px]">
                    <h2 className="uppercase text-primaryColor text-[32px] leading-[48px] font-bold mb-5 mt-6">CẤP SỐ MỚI</h2>
                    <p className="text-xl font-bold leading-[30px] mb-3">Dịch vụ khách hàng lựa chọn</p>
                    <FormControl sx={{ width: '400px', height: '44px' }}>
                        <InputLabel id="demo-multiple-name-label">Chọn dịch vụ</InputLabel>
                        <Select labelId="demo-multiple-name-label" label={'Chọn dịch vụ'} id="serviceId" value={selected} {...register?.('serviceId', { required: true })} onChange={handleChange}>
                            {options.map((option) => (
                    ...
                    <div className="flex items-center justify-center gap-6 mt-[80px]">
                        ...
                        <Button label="In số" onSubmit={handleSubmit(onSubmit)} disabled={isLoading} />
                    </div>
                </div>
            </div>
            {isOpen ? (
