...
const GiveNumber = () => {
    ...
    const getUserLogs = async () => {
        const q = query(collection(db, 'give-numbers'), orderBy('orderNumber', 'asc'))
        const snap = await getDocs(q)
        const giveNumbers = snap.docs.map((doc) => ({ ...doc.data(), uid: doc.id } as IGiveNumber))
        if (!snap.empty) {
            setGiveLogs([])
        }
    ...
    if (selected.status === 'pending') {
        giveNumbers = giveNumbers.filter((item) => item.status === selected.status)
    }
    ...
    return (
        <div className="w-full">
            ...
            <div className="flex items-center pl-6 mb-4 gap-6">
                <CustomSelect onChange={handleOnChangeSelected} options={[{ type: 'all', label: 'Tất cả' }, ...serviceNameList]} className="w-[150px]" name="serviceName" />
                <CustomSelect onChange={handleOnChangeSelected} options={optionCondition} className="w-[150px]" name="status" defaultValue={searchParams.get('selected') as string} />
            </div>
            <div className="flex gap-6">
                <Table columns={columns} data={giveNumbers} globalFilter={globalFilter} onGlobalFilterChange={setGlobalFilter} isLoading={isLoading} />
                ...
                {isLoading ? (
                    ...
                    <Manager icon={AddSquare} label="Cấp số mới" path="/give-number/give-list-numbers/new-number" />
                )}
            </div>
        </div>
    )
